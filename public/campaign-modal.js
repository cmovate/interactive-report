...
async function openCampaignModal(id){
  ...
  try{
    ...
    document.getElementById('cm-meta').textContent=(d.audience_type==='company'?'Company targeting':'People targeting')+' \u00b7 '+(d.account_id||'\u2014')+' \u00b7 '+(parseInt(d.contact_count)||0)+' contacts';
...
function closeCM(){document.getElementById('cm-back').style.display='none';cmId=cmData=null;}
...
async function loadCMTab(tab){
  ...
  try{
    if(tab==='analytics'){const d=await fetch('/api/campaigns/'+cmId+'/ab-analytics').then(r=>r.json());el.innerHTML=buildAnalyticsHTML(d);}
    else if(tab==='companies'){const d=await fetch('/api/campaigns/'+cmId+'/companies').then(r=>r.json());el.innerHTML=buildCompaniesHTML(d.items||[]);}
    else if(tab==='audience'){await loadCMContacts(1);}
    else if(tab==='settings'){el.innerHTML=buildSettingsHTML();}
...
function buildAnalyticsHTML(data){
  ...
  let html=`<div class="analytics-section-label">Overview</div><div class="overview-grid">
  ...
  return html+steps.map(step=>{
    ...
    const rows=vs.map(v=>{const best=isAB&&v.rate===mx&&mx>0;const bw=mx>0?Math.round(v.rate/mx*80):0;
...
function buildCompaniesHTML(companies){
  ...
  let tbl='';
  if(!companies.length){tbl='<div style="text-align:center;color:#aaa;padding:40px;background:#fafafa;border-radius:10px;">No companies in this campaign yet.</div>';}
  else{
    const rows=companies.map(co=>{const li=co.li_company_url?`<a class="cell-link" href="${esc(safeUrl(co.li_company_url))}" target="_blank" rel="noopener">LinkedIn \u2197</a>`:'<span style="color:#ccc;">\u2014</span>';const dt=co.created_at?new Date(co.created_at).toLocaleDateString():'\u2014';return`<tr><td style="font-weight:500;">${esc(co.company_name||'\u2014')}</td><td>${li}</td><td style="text-align:center;color:#888;">${co.contact_count||0}</td><td style="color:#aaa;font-size:12px;">${dt}</td></tr>`;}).join('');
...
async function addCMCompanies(){
  ...
  try{
    ...
    res.innerHTML=`<span style="color:#0F6E56;">\u2713 ${d.contacts_added} contacts added \u00b7 ${d.companies_searched} searched \u00b7 ${d.contacts_found} found</span>`;
    ta.value='';cmLoaded.companies=false;cmLoaded.audience=false;
    if(cmTab==='companies'){cmLoaded.companies=true;const d2=await fetch('/api/campaigns/'+cmId+'/companies').then(r=>r.json());document.getElementById('cmb-companies').innerHTML=buildCompaniesHTML(d2.items||[]);}
    load();
...
async function loadCMContacts(page){
  ...
  try{const d=await fetch(`/api/campaigns/${cmId}/contacts?page=${page}&limit=50`).then(r=>r.json());el.innerHTML=buildAudienceHTML(d);}
  catch(e){el.innerHTML=`<div class="tab-err">Error: ${esc(e.message)}</div>`;}
}
...
async function addCMContacts(){
  ...
  try{
    const r=await fetch(`/api/campaigns/${cmId}/contacts`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contacts:urls.map(li_profile_url=>({li_profile_url}))})});
    const d=await r.json();if(!r.ok)throw new Error(d.error);
    res.innerHTML=`<span style="color:#0F6E56;">\u2713 ${d.added} added \u00b7 ${d.skipped} skipped \u00b7 ${d.enrichment_queued} enrichment queued</span>`;
...
async function removeCMContact(cid){
  ...
  try{const r=await fetch(`/api/campaigns/${cmId}/contacts/${cid}`,{method:'DELETE'});if(!r.ok)throw new Error((await r.json()).error);await loadCMContacts(cmCtPage);load();}
...
async function toggleCMStatus(){
  ...
  try{const r=await fetch(`/api/campaigns/${cmId}/status`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:ns})});if(!r.ok)throw new Error((await r.json()).error);cmData.status=ns;const btn=document.getElementById('cm-st-btn');btn.textContent=ns==='active'?'\u23f8 Pause':'\u25b6 Resume';btn.style.color=ns==='active'?'#DC2626':'#1D9E75';load();}
  catch(e){alert('Failed: '+e.message);}
}
...
async function saveCMSettings(){
  ...
  try{const r=await fetch(`/api/campaigns/${cmId}/settings`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings:cmSettings})});if(!r.ok)throw new Error((await r.json()).error);if(cmData)cmData.settings=JSON.parse(JSON.stringify(cmSettings));btn.textContent='\u2713 Saved!';setTimeout(()=>{btn.disabled=false;btn.textContent='Save settings';},2000);load();}
