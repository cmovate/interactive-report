...
async function run() {
  if (ran) return;
  ran = true;
  console.log('[EMERGENCY] Starting Eshaan message deletion...');
  ...
  try {
    ...
    console.log(`[EMERGENCY] Found ${rows.length} Eshaan conversations`);
    ...
    for (const row of rows) {
      try {
        ...
        if (!outbound.length) {
          console.log(`[EMERGENCY] ${row.first_name}: no outbound msgs`);
          continue;
        }
    ...
    console.log('[EMERGENCY] Eshaan deletion complete');
  } catch(e) {
